...
$factory->define(App\Model\Activity::class, function (Faker $faker) {
    return [
      ...
      `end_date` => $faker->dateTimeBetween($startDate = 'now', $endDate = '1 month', $timezone = date_default_timezone_get()),
      `has_custom_dates` => $faker->boolean,
      `address` => $faker->address,
      `zipcode` => $faker->postcode,
      `city` => $faker->cityPrefix,
      `region` => $faker->state,
      `country_id` => $faker->state,
      `website` => $faker->,
      `description` => $faker->paragraph,
      `user_id` => numberBetween(1, 50),
      ...
      `end_registration` => $faker->dateTimeBetween($startDate = 'now', $endDate = '12 hours', $timezone = date_default_timezone_get()),
      `max_registrations` => $faker->numberBetween(10, 20),
      `icon_path` => $faker->imageUrl($width = 640, $height = 480),
      `vat_percentage` => $faker->numberBetween(10, 20),
      `type_of_activity` => $faker->word,
      `organizer_type` => $faker->word, varchar(12) NOT NULL,
      `club_id` => function () {
